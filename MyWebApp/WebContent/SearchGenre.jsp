...
<a href="SearchGenre.html"><button type="button">Back</button></a>
...
<%
	...
	if(sort!=null){
	   ...
	   Connection conn;
				...
				ResultSet rs=stmt.executeQuery("select item.item_id, item.title, item.author, item.genre, item.isbn, item.publisher, item.year, item.description, item.item_cond, active_inventory.item_price from item, active_inventory where genre='"+genre+"' and item.item_id=active_inventory.item_id order by "+sort+"");
